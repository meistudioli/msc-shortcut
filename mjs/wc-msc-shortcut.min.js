import{_wcl}from"./common-lib.js";import{_wccss}from"./common-css.js";import Mustache from"./mustache.js";const defaults={groups:[]},booleanAttrs=[],objectAttrs=["groups"],custumEvents={click:"msc-shortcut-click",toggle:"msc-shortcut-toggle"},isAnchorPositioningAPIReady=CSS.supports("anchor-name: --myanchor"),template=document.createElement("template"),templateGroups=(template.innerHTML=`
<style>
${_wccss}
...
${Array(30).fill().map((t,a)=>{a+=1;return`
        &[data-sn="${a}"] {
          --icon: var(--msc-shortcut-button${a}-icon, var(--shortcut-button-icon));
        }`}).join("\n")}
...
`,document.createElement("template"));if(templateGroups.innerHTML=`
{{#groups}}
...
{{/groups}}
`,CSS?.registerProperty)try{CSS.registerProperty({name:"--msc-shortcut-trigger-size",syntax:"<number>",inherits:!0,initialValue:40}),CSS.registerProperty({name:"--msc-shortcut--gap",syntax:"<length>",inherits:!0,initialValue:"12px"}),CSS.registerProperty({name:"--msc-shortcut-shortcut-legend-color",syntax:"<color>",inherits:!0,initialValue:"rgba(151 158 168)"}),CSS.registerProperty({name:"--msc-shortcut-shortcut-text-color",syntax:"<color>",inherits:!0,initialValue:"rgba(35 42 49)"}),CSS.registerProperty({name:"--msc-shortcut-shortcut-line-color",syntax:"<color>",inherits:!0,initialValue:"rgba(224 228 233)"})}catch(t){console.warn("msc-shortcut: "+t.message)}class MscShortcut extends HTMLElement{#data;#nodes;#config;constructor(t){super(),this.attachShadow({mode:"open"}),this.shadowRoot.appendChild(template.content.cloneNode(!0)),this.#data={controller:""},this.#nodes={styleSheet:this.shadowRoot.querySelector("style"),main:this.shadowRoot.querySelector(".main"),trigger:this.shadowRoot.querySelector(".trigger"),shortcut:this.shadowRoot.querySelector(".shortcut")},this.#config={...defaults,...t},this._onTriggerClick=this._onTriggerClick.bind(this),this._onShortcutClick=this._onShortcutClick.bind(this),this._onToggle=this._onToggle.bind(this)}async connectedCallback(){var{config:t,error:a}=await _wcl.getWCConfig(this),{trigger:e,shortcut:o}=this.#nodes;a?(console.warn(_wcl.classToTagName(this.constructor.name)+": "+a),this.remove()):(this.#config={...this.#config,...t},Object.keys(defaults).forEach(t=>this.#upgradeProperty(t)),this.#data.controller=new AbortController,a=this.#data.controller.signal,e.addEventListener("click",this._onTriggerClick,{signal:a}),o.addEventListener("click",this._onShortcutClick,{signal:a}),o.addEventListener("toggle",this._onToggle,{signal:a}))}disconnectedCallback(){this.#data?.controller&&this.#data.controller.abort()}#format(e,t,r){if(null!==r)switch(e){case"groups":{let a;try{a=JSON.parse(r)}catch(t){console.warn(_wcl.classToTagName(this.constructor.name)+": "+t.message),a=Array.isArray(defaults[e])?[...defaults[e]]:{...defaults[e]}}let o=0;a=a.reduce((t,a,e)=>{var{legend:a="Legend - "+(e+1),menu:e=[]}=a;return e.length?t.concat({legend:a,menu:e.map((t={})=>{o+=1;var{key:t="key-"+o,title:a="title-"+o,content:e="content-"+o}=t;return{key:t,title:a,content:e,sn:o}})}):t},[]),this.#config[e]=a;break}}else booleanAttrs.includes(e)?this.#config[e]=!1:this.#config[e]=defaults[e]}attributeChangedCallback(t,a,e){MscShortcut.observedAttributes.includes(t)&&(this.#format(t,a,e),"groups"===t)&&(a=this.#nodes["shortcut"],a.replaceChildren(),isAnchorPositioningAPIReady?a.popover="auto":a.toggleAttribute("popover",!1),e=Mustache.render(templateGroups.innerHTML,{groups:this.groups}),a.insertAdjacentHTML("beforeend",e))}static get observedAttributes(){return Object.keys(defaults)}static get supportedEvents(){return Object.keys(custumEvents).map(t=>custumEvents[t])}#upgradeProperty(t){let a;MscShortcut.observedAttributes.includes(t)&&(Object.prototype.hasOwnProperty.call(this,t)?(a=this[t],delete this[t]):a=booleanAttrs.includes(t)?!(!this.hasAttribute(t)&&!this.#config[t]):objectAttrs.includes(t)?this.hasAttribute(t)?this.getAttribute(t):JSON.stringify(this.#config[t]):this.hasAttribute(t)?this.getAttribute(t):this.#config[t],this[t]=a)}set groups(t){t?(t=[..."string"==typeof t?JSON.parse(t):t],this.setAttribute("groups",JSON.stringify(t))):this.removeAttribute("groups")}get groups(){return this.#config.groups}get open(){var{main:t,trigger:a}=this.#nodes;return!!(isAnchorPositioningAPIReady?t.querySelector("*:popover-open"):a.hasAttribute("data-reverse"))}#fireEvent(t,a){this.dispatchEvent(new CustomEvent(t,{bubbles:!0,composed:!0,...a&&{detail:a}}))}_onTriggerClick(){var t;isAnchorPositioningAPIReady||(t=!this.open,this.#fireEvent(custumEvents.toggle,{oldState:t?"closed":"open",newState:t?"open":"closed"}),this.#nodes.trigger.toggleAttribute("data-reverse"))}_onShortcutClick(t){t=t.target.closest("button");t&&(isAnchorPositioningAPIReady||(this.#fireEvent(custumEvents.toggle,{oldState:"open",newState:"closed"}),this.#nodes.trigger.toggleAttribute("data-reverse",!1)),this.#fireEvent(custumEvents.click,{key:t.dataset.key,title:t.title,content:t.textContent.trim()}))}_onToggle(t){var{newState:t,oldState:a}=t;this.#fireEvent(custumEvents.toggle,{newState:t,oldState:a})}toggle(t=!this.open){(t=Boolean(t))!==this.open&&this.#nodes.trigger.click()}click(){this.#nodes.trigger.click()}}const S=_wcl.supports(),T=_wcl.classToTagName("MscShortcut");S.customElements&&S.shadowDOM&&S.template&&!window.customElements.get(T)&&window.customElements.define(_wcl.classToTagName("MscShortcut"),MscShortcut);export{MscShortcut};